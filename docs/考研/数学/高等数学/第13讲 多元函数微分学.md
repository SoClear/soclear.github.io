...
例 13.9 设 $z = f\left( {{\mathrm{e}}^{x}\sin y,{x}^{2} + {y}^{2}}\right)$ ,其中 $f$ 具有二阶连续偏导数, ${f}_{1}^{\prime}\left( {0,0}\right) = 1,{f}_{2}^{\prime}\left( {0,0}\right) = - 1$ ,则 ${\left. \frac{{\partial }^{2}z}{\partial x\partial y}\right| }_{\left( 0,0\right) } = \left( \;\right) .$
...
**对后者考虑夹逼准则** 根据 $a^2+b^2 \ge 2ab$ ，得 $x^4 + y^2 \ge 2x^2|y|$ ，所以 $0 \le  \mathop{\lim }\limits_{{\left( {x, y}\right) \rightarrow \left( {0,0}\right) }}\frac{{x}^{3}y}{{x}^{4} + {y}^{2}} \le \mathop{\lim }\limits_{{\left( {x, y}\right) \rightarrow \left( {0,0}\right) }}|\frac{{x}^{3}y}{2x^2y}|=\mathop{\lim }\limits_{x\to0}|\frac{x}{2}|=0$ ，由夹逼准则得 $\mathop{\lim }\limits_{{\left( {x, y}\right) \rightarrow \left( {0,0}\right) }}\frac{{x}^{3}y}{{x}^{4} + {y}^{2}}= 0$

**对后者考虑极坐标**. 令 $x = r\cos \theta, y = r\sin \theta$ ,有

...
- 前 $n$ 项的和$S_n=\frac n2\left[2a_1+\left(n-1\right)d\right]=\frac n2\left(a_1+a_n\right).$
...
- 前$n$项的和$S_n=\begin{cases}na_1,&r=1,\\\frac{a_1\left(1-r^n\right)}{1-r},&r\neq1.\end{cases}$

- 常用$1+r+r^{2}+\cdots+r^{n-1}=\frac{1-r^{n}}{1-r}\left(r\neq1\right).$
...
- $\sqrt{ab}\leq\frac{a+b}{2}\leq\sqrt{\frac{a^{2}+b^{2}}{2}}\left(a,b\geqslant0\right);$

    还有$\left|ab\right|\leqslant\frac{a^{2}+b^{2}}{2}$,例如，若$u_n>0,则\frac{u_{n}}{n}=u_{n}\cdot\frac{1}{n}\leqslant\frac{u_{n}^{2}+\frac{1}{n^{2}}}{2}$

- $\sqrt[3]{abc}\leq\frac{a+b+c}{3}\leq\sqrt{\frac{a^{2}+b^{2}+c^{2}}{3}}\left(a,b,c\geqslant0\right).$

***2.3***

设 $a\geq b\geqq0$ ,则 $\begin{cases}当m>0时，a^{m}\geq b^{m},\\当m<0时，a^{m}\leq b^{m}.\end{cases}$

***2.4***

若 $0<a<x<b,0<c<y<d,则\displaystyle\frac{c}{b}<\frac{y}{x}<\frac{d}{a}$

考研中考过：当 $n\pi<x<\left(n+1\right)\pi,2n<S\left(x\right)<2\left(n+1\right)时，\frac{2n}{\left(n+1\right)\pi}<\frac{S\left(x\right)}{r}<\frac{2\left(n+1\right)}{n\pi}$
...
考研中考过：当$x_n>0时，x_{n+1}=\sin x_{n}<x_{n},故\left\{x_{n}\right\}$ 单调减少。

***2.7***

$当0<x<\frac{\pi}{4}时，x<\tan x<\frac{4}{\pi}x$

***2.8***

$当0<x<\frac{\pi}{2}时，\sin x>\frac{2}{\pi}x$

***2.9***

$\arctan x\leq x\leq\arcsin x\left(0\leq x\leq1\right).$

可考：当$x_n>0$时，$x_n+1=\arctan x_n<x_n$,故$\{x_n\}$单调减少

***2.10***

$e^{x}\geq x+1\left(\forall x\right).$

可考：当$x_n+1=\mathrm{e}^{x_n}-1$时，由$\mathrm{e}^{x_n-1}\geqslant x_n$,得$x_{n+1}\geqslant x_n$,即$\{x_n\}$单调不减.
...
**证明** $|x_{n+1}-a|=\left|f(x_n)-f(a)\right|$ ,应用拉格朗日中值定理： ${\left|f^{\prime}(\xi)\right|\left|x_n-a\right|\leq k\left|x_n-a\right|,\text{其中 }\xi\text{介于}a\text{与 }x_n\text{之间，}}$
